...
float4 materialAmbient = float4(0.9f, 0.9f, 0.9f, 1.0f);  
...
const float4 lightSpecular = float4(0.6f, 0.6f, 0.6f, 1.0f);
const float4 lightAttenuation = float4(0.0f, 0.01f, 0.0f, 1.0f);
						     ...
						     float3 camToVertex)
{
	 ...
	 for (int i = 0; i < NUMBER_OF_LIGHTS; i++)
	 {
			 ...
			 ((i == 2) && lightEnabled3) )
		 {
			 ...
			 if (i == 0)
			 {
				 shadowMapDepth = texCUBEproj(cubeShadowMapSampler, float4(-(PLightDirection.xyz), 0.0f)).x;
			 }
			 if (i == 1)
			 {
				 shadowMapDepth = texCUBEproj(cubeShadowMapSampler2, float4(-(PLightDirection.xyz), 0.0f)).x;
			 }
			 if (i == 2)
			 {
				 shadowMapDepth = texCUBEproj(cubeShadowMapSampler3, float4(-(PLightDirection.xyz), 0.0f)).x;
			 }

			 if (distance <= shadowMapDepth)

				 //if (distance - shadowMapDepth <= 5)
			 {
